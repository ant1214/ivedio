...
// import 'package:ivideo/core/services/watch_history_service.dart'; // 添加这行导入
...
//   bool _historyAdded = false; // 添加标记，防止重复记录
...
//    _addToWatchHistory(); // 添加观看记录
//   }
// // 在 _initializeVideoPlayer 中添加进度监听
...
//       // // 视频开始播放时记录
//       // if (isPlaying && !_historyAdded) {
//       //   print('🎬 视频开始播放，添加初始观看记录');
//       //   _addToWatchHistory();
//       //   _historyAdded = true;
//       // }
...
// //     print('正在添加观看记录，视频ID: ${widget.video.id}');
    
// //     // 使用正确的参数调用
// //     final success = await WatchHistoryService.addWatchHistory(
// //       widget.video.id,
// //       progress: 0, // 初始进度为0
// //       duration: widget.video.duration ??0, // 使用视频的总时长
// //     );
...
//       print('正在处理观看数据，视频ID: ${widget.video.id}');
      
//       // 1. 增加观看次数（所有用户都统计，但只执行一次）
//       if (!_viewCountIncremented) {
//         print('正在增加观看次数...');
//         final viewSuccess = await VideoService.incrementViewCount(widget.video.id);
//         if (viewSuccess) {
//           print('✅ 观看次数增加成功');
